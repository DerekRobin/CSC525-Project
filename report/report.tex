...
The methodology workflow we followed is presented in figure \ref{fig:methods}.
...
In order to find the Alu domains present in our sequences, we used Dfam website \cite{storer2021dfam}. 
...
By using the sequence search we find the Alu domains of the BC200 and its homologs. We performed a sequence search for each of the target lnc RNAs, then by looking in the result tab we compared the first row name of the diagram for each sequence search to others, if they have the same name it means that they have the same Alu elements. 
...
The output format is in ClustalW, and the guide tree is shown below. By counting the number of nucleotides peresent in the human BC200 and then counting the number of nucleotides which are conserved, we calculated the conservation percentage.
...
Finding the Alu domain of BC200 and its homologs (see supplementary information), we concluded that the Alu domain between human BC200 and its Selected homologs are nearly identical. So, their function in brain may be same with each other. 
...
\section{ClustalW Multiple Sequence Alignment of BC200 and Hominoidea Homologs}
\label{app:A}
